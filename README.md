...
3. **Run the migration preflight** – Execute `scripts/postgres-migration/preflight.sh --config /root/config/vs-conf.txt`. The agent:
   ...
   - Confirms the database is reachable, then activates the fallback external connector service (`POSTGRES_EXTERNAL_CONNECTOR_SERVICE`, defaulting to `visionsuit-external-connector`) so a standby consumer can attach immediately if the tunnel fails later.
