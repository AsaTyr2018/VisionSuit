...
- **Self-hosted NSFW roadmap** – The [NSFW Moderation Deployment Plan](docs/nsfw-deployment-plan.md) outlines how metadata scoring, OpenCV-style heuristics (no nude_vs_swimwear.onnx dependency), and admin tuning controls combine into the next-generation moderation stack, and the backend now executes those heuristics on every upload while storing moderation summaries for review.
