...
3. **Run the migration preflight** – Execute `scripts/postgres-migration/preflight.sh --config /root/config/vs-conf.txt`. The agent imports the key, stores a copy under `~/.ssh/visionsuit-migration`, loads it into `ssh-agent`, writes the agent environment to `~/.ssh/visionsuit-migration-agent.env`, establishes SSH connectivity, confirms the database is reachable, verifies that at least `pgloader` or `sqlite3` is installed locally (and attempts to install missing tools with the detected package manager), and writes `.env-migration` with the derived PostgreSQL and SQLite settings required for the next steps.
