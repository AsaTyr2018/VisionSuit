...
1. **Preparation on the target host** – Run `scripts/postgres-migration/postgress-prepare.sh` on the future PostgreSQL server. The helper validates OS compatibility, installs PostgreSQL, provisions a `visionsuit-migrator` Linux user with root-capable SSH key access, creates the `visionsuit` database, and emits `/root/vs-conf.txt` containing the SSH key (base64), database credentials, the server's primary IPv4 address for SSH and Prisma connections, and the internal hostname used for SSH tunnels. It also writes a sudoers drop-in so the automation account can escalate without interactive passwords.
   When a rehearsal needs to be unwound, run `scripts/postgres-migration/postgress-rollback.sh --force` with the same identifiers to remove the automation user, generated SSH material, configuration bundle, PostgreSQL role, and database.
...
3. **Run the migration preflight** – Execute `scripts/postgres-migration/preflight.sh --config /root/config/vs-conf.txt`. The agent:
   - Imports the key, stores a copy under `~/.ssh/visionsuit-migration`, loads it into `ssh-agent`, and writes the agent environment to `~/.ssh/visionsuit-migration-agent.env` for reuse.
   - Establishes SSH connectivity and ensures the PostgreSQL role declared in the bundle exists (creating it or resetting the password through the exported superuser when necessary).
  - Confirms the database is reachable, then verifies the fallback external connector service (`POSTGRES_EXTERNAL_CONNECTOR_SERVICE`, defaulting to `visionsuit-external-connector`) exists before enabling it—prompting you to install or rename the unit when it is missing so standby consumers can attach immediately once the tunnel validation succeeds.
   - Verifies that at least `pgloader` or `sqlite3` is installed locally, attempting installation through the detected package manager when either tool is missing.
   - Writes `.env-migration` with shell-escaped SQLite/PostgreSQL settings—including a URI-encoded `DATABASE_URL`—for the next steps.
4. **Migrate data** – Launch `scripts/postgres-migration/migration.sh`. It snapshots the SQLite file, opens an SSH tunnel with the credentials from `.env-migration` (set `MIGRATION_SKIP_TUNNEL=1` when a direct TCP route is available), clears any stale PostgreSQL tables, performs the transfer with `pgloader` (or a `sqlite3`/`psql` fallback), and validates row counts across every table. If the tunnel import fails—for example because `pgloader` rejects the forwarded credentials—the helper automatically tears down the tunnel and retries against the direct `POSTGRES_HOST` address before giving up; set `MIGRATION_DIRECT_FALLBACK=0` to skip the retry. The command aborts immediately if a mismatch appears.
   The helper now validates that both the configured PostgreSQL port and the resolved tunnel port are numeric before attempting to connect so libpq never confuses the password for the port when credentials drift. It also percent-encodes PostgreSQL credentials inside the generated pgloader configuration, so complex usernames or passwords (including `@`, spaces, or shell metacharacters) import cleanly without manual escaping.
