...
- **On-Site Generator hub** – Role-aware wizard that lists curated base models defined in the admin console, lets curators compose prompts, mix LoRAs, tap trigger suggestions straight into the prompt box, pick dimensions, and enqueue generation jobs while preserving per-user history.
...
- Administrators can toggle the On-Site Generator between an **Admin only** preview phase and a **Members & curators** rollout from the Administration → Generator tab, and curate the exact base-model list consumed by the wizard without touching storage manifests.
