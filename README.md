...
- **On-Site Generator hub** – Role-aware wizard that mirrors the admin-curated base-model labels from Administration → Generator, exposes them in a checkbox matrix, trusts configured base models even when no catalog asset exists (pulling the checkpoint straight from the GPU node store), lets curators compose prompts, mix LoRAs, tap trigger suggestions straight into the prompt box, pick dimensions, and enqueue generation jobs while preserving per-user history and queue metrics. The backend now serializes dispatches so only one render executes at a time across the platform while live telemetry surfaces GPU activity, pause/resume state, and any admin-imposed generation blocks. Generated artifacts stream through the backend proxy with automatic session-token passthrough so history thumbnails and downloads never leak private MinIO hostnames.
