...
3. **Run the migration preflight** – Execute `scripts/postgres-migration/preflight.sh --config /root/config/vs-conf.txt`. The agent imports the key, stores a copy under `~/.ssh/visionsuit-migration`, loads it into `ssh-agent`, writes the agent environment to `~/.ssh/visionsuit-migration-agent.env`, establishes SSH connectivity, confirms the database is reachable, and writes `.env-migration` with the derived PostgreSQL and SQLite settings required for the next steps.
