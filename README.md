...
- **On-Site Generator hub** – Role-aware wizard that lists curated base models sourced directly from the database, lets curators compose prompts, mix LoRAs, tap trigger suggestions straight into the prompt box, pick dimensions, and enqueue generation jobs while preserving per-user history.
