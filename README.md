...
3. **Run the migration preflight** – Execute `scripts/postgres-migration/preflight.sh --config /root/config/vs-conf.txt`. The agent imports the key, stores a copy under `~/.ssh/visionsuit-migration`, loads it into `ssh-agent`, writes the agent environment to `~/.ssh/visionsuit-migration-agent.env`, establishes SSH connectivity, ensures the PostgreSQL role declared in the bundle exists (creating it or resetting the password through the exported superuser when necessary), confirms the database is reachable, verifies that at least `pgloader` or `sqlite3` is installed locally (and attempts to install missing tools with the detected package manager), and writes `.env-migration` with shell-escaped SQLite/PostgreSQL settings—including a URI-encoded `DATABASE_URL`—for the next steps.
