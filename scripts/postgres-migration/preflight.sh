...
POSTGRES_EXTERNAL_CONNECTOR_SERVICE="$(parse_conf POSTGRES_EXTERNAL_CONNECTOR_SERVICE)"

if [[ -z "$POSTGRES_EXTERNAL_CONNECTOR_SERVICE" ]]; then
  POSTGRES_EXTERNAL_CONNECTOR_SERVICE="visionsuit-external-connector"
fi
...
log "Activating fallback external connector service ${POSTGRES_EXTERNAL_CONNECTOR_SERVICE}."
if ! "${SSH_CMD[@]}" \
  "${SSH_USER}@${SSH_HOST}" \
  SERVICE="$POSTGRES_EXTERNAL_CONNECTOR_SERVICE" \
  bash -s <<'EOS'; then
set -euo pipefail
if [[ -z "${SERVICE}" ]]; then
  echo "[preflight] External connector service name is empty." >&2
  exit 1
fi
...
printf '# Autogenerated by scripts/postgres-migration/preflight.sh\n' >"$ENV_MIGRATION_FILE"

write_env_assignment SQLITE_PATH "$SQLITE_PATH"
write_env_assignment POSTGRES_SSH_HOST "$SSH_HOST"
write_env_assignment POSTGRES_SSH_PORT "$SSH_PORT"
write_env_assignment POSTGRES_SSH_USER "$SSH_USER"
write_env_assignment POSTGRES_SSH_KEY "$SSH_KEY_PATH"
write_env_assignment POSTGRES_HOST "$POSTGRES_HOST"
write_env_assignment POSTGRES_PORT "$POSTGRES_PORT"
write_env_assignment POSTGRES_DB "$POSTGRES_DB"
write_env_assignment POSTGRES_USER "$POSTGRES_USER"
write_env_assignment POSTGRES_PASSWORD "$POSTGRES_PASSWORD"
write_env_assignment POSTGRES_SUPERUSER "$POSTGRES_SUPERUSER"
write_env_assignment POSTGRES_INTERNAL_HOST "$POSTGRES_INTERNAL_HOST"
write_env_assignment POSTGRES_EXTERNAL_CONNECTOR_SERVICE "$POSTGRES_EXTERNAL_CONNECTOR_SERVICE"

ENCODED_POSTGRES_USER="$(uri_encode_component "$POSTGRES_USER")"
ENCODED_POSTGRES_PASSWORD="$(uri_encode_component "$POSTGRES_PASSWORD")"
ENCODED_POSTGRES_DB="$(uri_encode_component "$POSTGRES_DB")"
DATABASE_URL="postgresql://${ENCODED_POSTGRES_USER}:${ENCODED_POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${ENCODED_POSTGRES_DB}"
